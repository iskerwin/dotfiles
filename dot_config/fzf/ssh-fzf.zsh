...
_fzf_complete_ssh() {
    _fzf_complete --ansi --border --cycle \
        ...
        --preview 'host=$(echo {} | awk "{print \$1}"); 
              ...
              grep -i -E "^(hostname|port|user|identityfile|controlmaster|forwardagent|localforward|remoteforward|proxycommand|serveraliveinterval|serveralivecountmax|tcpkeepalive|compression|controlpath|controlpersist) " |
              sed -E '\''
                  s/^hostname/HostName/
                  s/^port/Port/
                  s/^user/User/
